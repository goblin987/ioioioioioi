    ...
    async def initialize(self):
        ...
        try:
            ...
            self.client = Client(
                ...
                workdir=".",
                session_string=None  # Let Pyrogram handle session file
            )
    ...
    async def send_secret_message(self, user_id: int, message: str, media_files: List[str] = None):
        ...
        try:
            ...
            try:
                ...
                if not user_entity or not hasattr(user_entity, 'access_hash') or not user_entity.access_hash:
                    logger.error(f"❌ USERBOT: Cannot get access hash for user {username}")
                    return False
            except Exception as e:
                logger.error(f"❌ USERBOT: Error getting user {username}: {e}")
                return False
            
            # Create secret chat using raw API
            try:
                ...
                if not result or not hasattr(result, 'chat_id'):
                    logger.error(f"❌ USERBOT: Failed to create secret chat with user {user_id}")
                    return False
                
                secret_chat_id = result.chat_id
                logger.info(f"✅ USERBOT: Secret chat created with ID: {secret_chat_id}")
                
            except Exception as e:
                logger.error(f"❌ USERBOT: Error creating secret chat: {e}")
                return False
            ...
            try:
                ...
                logger.error(f"❌ USERBOT: Error sending text message: {e}")
                return False
